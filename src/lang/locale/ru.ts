...
`,
  ...
  AGGRESSIVE_PALM_REJECTION_DESC:
    "Агрессивно подавляет случайные касания пальцем или ладонью при рисовании пером/S-Pen. Рекомендуется для планшетов. " +
    "Временно отключает панорамирование пальцем во время рисования и сразу после контакта с пером (в пределах окна ожидания), " +
    "а также отклоняет большие области контакта (например, прислоненную ладонь).",
  ...
  PALM_REJECTION_THRESHOLD_DESC:
    "Сенсорный ввод с шириной или высотой области контакта больше этого порога будет рассматриваться как касание ладонью и блокироваться. Значение 0 полностью отключает сенсорную прокрутку (режим «только перо»).",
  ...
  EMBED_IMAGE_CACHE_DESC:
    "Кэшируйте изображения для вставки в markdown. Это ускорит процесс встраивания, но в случае, если вы составите изображения из нескольких чертежей-субкомпонентов, " +
